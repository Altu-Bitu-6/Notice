 ...
 * !주의! 입구(0)는 항상 1번 건물과 연결된 도로를 가짐(입력도 먼저 주어짐)
 ...
 */
 ...
 int main() {
     ...
     int min_stress = kruskal(n, edge) + (1 - ew);  // 0-1번 연결 간선의 가중치 추가
     
     // 최댓값 (오르막(1)으로 먼저 가야 함)
     sort(edge.begin(), edge.end(), greater<>()); // 내림차순
     int max_stress = kruskal(n, edge) + (1 - ew); // 0-1번 연결 간선의 가중치 추가
